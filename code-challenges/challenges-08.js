...
const customerAndAge = (obj) => {
  ...
  let count = 0;
  for (const i in obj) {
    arr[count] = `Customer Name :${i} , Age :${obj[i]}`;
    count++;
  }
  return arr;
};
...
const getEntries = (obj) => {
  ...
  let count = 0;
  let arr2 = [];
  // for(const element of recipeInfo.ingredients){
  //   ingred+=`${element},` ;
  //}
  for (const i in obj) {
    arr2[count] = `${i}: ${obj[i]}`;
...
const getInfo = (arr) => {
  ...
  // write your code here
  for (const i of arr) {
    coursesName[count] = i.course;
    studentsName = studentsName.concat(i.Students);
    count++;
  }
...
for(const element of arr){
    ...
    for(const i in element.Students){
        //console.log(element.Students[i]);
        if(element.Students[i]=='Kalé' || element.Students[i]=='Alisha' || element.Students[i]=='Alphonso' || element.Students[i]=='Briana'){
            arr2[counter]={
